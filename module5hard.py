...
class UrTube:
    ...
    def log_in(self, nickname, password):
        ...
        for user in self.users:
            if user.nickname == nickname and password == hashed_password:
                self.current_user = user
                print(f'Пользователь {nikename} вошел в систему.')
                return True
        print('Ошибка входа: неверный логин или пароль.')
        return False

    def register(self, nikename, password, age):
        new_user = User(nikename, password, age)
        if new_user in self.users:
            print(f'Пользователь {nikename} уже существует.')
